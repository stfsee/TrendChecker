  ...
  String indicators = "#indicatorsBelowChart=ADX&useFixAverage=true&fixAverage1=200&fixAverage0=38&e&"
	...
	void checkInvertedHammer()
	{
		if (hasInvertedHammerShape())
		{
			...
			this.candleInfo = candleInfo+" InvertedHammer "+Util.formattedDate(last.date)
		}
	}
	...
	String getOutputLinesWithTrends(String title) {
		String trendDiffFormatted = sprintf("%.2f", trendDiff*100)
		String currentPriceFormatted = sprintf("%.2f", currentPrice)
		checkCandles()
		StringBuffer outLine = new StringBuffer("<tr><td>")
		outLine.append title + "</td>"
		outLine.append "<td><a href=\"http://www.comdirect.de/inf/aktien/detail/chart.html?ID_NOTATION="+stock.comdNotationId+"&timeSpan="+period+"M"+indicators+"\" target=\"_blank\">"+stock.name+"</a></td>"
		outLine.append "<td>"+currentPriceFormatted+"</td>"
		outLine.append "<td>"+formattedStartDate+"</td>"
		outLine.append "<td>"+formattedLastValue+"</td>"
		//outLine.append(", Letzter Close: ")
		//outLine.append last.close + " "
		outLine.append ("<td><font color=\"#FF0000\"><div title=\"")
		...
		outLine.append '<td><span title="Varianzen der letzten 4 Wochen, basierend auf ROC">'+sprintf("%.2f", fourWVar) + "</span></td>"
		outLine.append '<td><span title="Performance/Varianz der letzten 4 Wochen">'+sprintf("%.2f", fourWP*100/fourWVar) + "</span></td>"
		double fourWDailyVar = getFourWDailyVar()
		outLine.append '<td><span title="Mittelwert der täglichen Varianzen der letzten 4 Wochen">'+sprintf("%.2f", fourWDailyVar)
		outLine.append("</span></td></tr>") 
		//outLine.append('</br>\n')
		return outLine
	}
